...
output_result_path = "./design_review/DC_RC_113.md"
...
review_score_prompt = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            """
            To evaluate the individual review comments based on the criteria you've provided, I will need to analyze each comment in terms of its impact on design flow and its importance and informativeness. Since you have not provided specific review comments, I will outline a general approach to how you can evaluate them.

            ### Evaluation Criteria

            1. **Impact on Design Flow (1-10)**
            - **1-3**: Minimal impact; the comment is more of a suggestion or preference.
            - **4-6**: Moderate impact; the comment addresses an important aspect but is not critical.
            - **7-9**: High impact; the comment is essential for the design flow and should be implemented.
            - **10**: Critical impact; the comment is mandatory and must be applied to ensure functionality or compliance.

            2. **Importance and Informativeness (1-10)**
            - **1-3**: Lacks detail; does not provide significant insight into the project.
            - **4-6**: Somewhat informative; provides useful information but lacks depth.
            - **7-9**: Very informative; offers valuable insights that enhance understanding of the project.
            - **10**: Extremely informative; provides critical insights that are essential for understanding the system.

            ### Example Evaluation

            Here’s how you might evaluate a few hypothetical review comments:

            1. **Comment**: "Consider adding error handling for the API response."
            - **Impact on Design Flow**: 7 (Error handling is important for robustness)
            - **Importance and Informativeness**: 6 (Useful suggestion, but not deeply informative)
            - **Overall Score**: 6.5 (rounded to 7)

            2. **Comment**: "The user interface should be more intuitive to enhance user experience."
            - **Impact on Design Flow**: 5 (Important for user satisfaction but not critical)
            - **Importance and Informativeness**: 7 (Provides insight into user experience)
            - **Overall Score**: 6

            3. **Comment**: "The generative AI seems to play a critical role in the system, providing maturity and advancement level calculations, as well as creating considerations and improvement prompts. It's good to see AI being utilized in this way."
            - **Impact on Design Flow**: 8 (Understanding AI's role is important for design)
            - **Importance and Informativeness**: 10 (Highly informative about the system's functionality)
            - **Overall Score**: 9

            4. Average overall score : 8.15 (Average of Overall Score)
...
avg_review_score_prompt = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            """
            Iterate all review comments score and calculate average score for input {arr_rc_score}
            """,
        ),
        (
            "human",
            """
            Calculate collective average of average score for review comment collected by team leader.
            """,
        ),
    ]
)
avg_review_score_chain = (
    RunnableParallel(arr_rc_score=itemgetter("arr_rc_score"))
    | avg_review_score_prompt
    | model
    | output_parser
)

review_comments_score_arr = []
with open(output_result_path, "w") as f:
    for i in range(10):
        print(
            "-------Review comments for iteration ----------------",
            file=f,
        )
        ...
        review_comments_score_arr.append(review_comments_score)
    ...
    avg_review_comments_score = avg_review_score_chain.invoke(
        {"arr_rc_score": review_comments_score_arr}
    )

    print("---------avg_review_comments_score--------------", file=f)
    print(avg_review_comments_score, file=f)
    print(avg_review_comments_score)
